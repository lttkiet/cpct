...
protected:
    ...
    void SetUp() override {
        tmp_dir_ = (fs::temp_directory_path() / "cpct_gzip_test").string();
        fs::create_directories(tmp_dir_);
        test_file_ = tmp_dir_ + "/hello.txt";
        archive_   = tmp_dir_ + "/test.gz";
        std::ofstream(test_file_) << "Hello, GZIP test!";
    }
...
TEST_F(GzipTest, CreateAndExtract) {
...
TEST_F(GzipTest, ReadEntry) {
    ...
    EXPECT_EQ(content, "Hello, GZIP test!");
}
