...
protected:
    ...
    void SetUp() override {
        tmp_dir_ = (fs::temp_directory_path() / "cpct_zstd_test").string();
        fs::create_directories(tmp_dir_);
        test_file_ = tmp_dir_ + "/hello.txt";
        archive_   = tmp_dir_ + "/test.zst";
        std::ofstream(test_file_) << "Hello, ZSTD test!";
    }
...
TEST_F(ZstdTest, CreateAndExtract) {
...
TEST_F(ZstdTest, ReadEntry) {
    ...
    EXPECT_EQ(content, "Hello, ZSTD test!");
}
